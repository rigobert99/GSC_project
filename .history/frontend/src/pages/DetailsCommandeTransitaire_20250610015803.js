...
export default function DetailsCommandeTransitaire() {
    ...
    const fetchCommande = async () => {
      try {
        ...
        const transformedData = {
          ...
          numero: data.numero || "",
          total: data.total || 0,
          fournisseurs: data.fournisseurs ? data.fournisseurs.map(fournisseur => ({
            nom: fournisseur.nom || "",
            contact: fournisseur.contact || "",
            produits: fournisseur.produits ? fournisseur.produits.map(produit => ({
              nom: produit.nom || "",
              quantite: produit.quantite || 0,
              prix_unitaire: produit.prix_unitaire || 0,
              total: (produit.quantite * produit.prix_unitaire) || 0
            })) : []
          })) : []
        };
  ...
  const calculTotal = () => {
    ...
    return commande.fournisseurs.reduce((acc, f) => {
      return acc + f.produits.reduce((pAcc, p) => pAcc + p.total, 0);
    }, 0).toFixed(2);
  };
